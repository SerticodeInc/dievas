...
void main() {
  group('DievasBadge', () {
    testWidgets('renders without throwing — default args', (tester) async {
      await tester.pumpWidget(_harness(const DievasBadge(label: 'New')));
    ...
    testWidgets('renders all styles', (tester) async {
      for (final style in DievasBadgeStyle.values) {
        await tester.pumpWidget(_harness(DievasBadge(label: style.name, style: style)));
        expect(tester.takeException(), isNull);
      }
    });
    ...
    testWidgets('renders all tones', (tester) async {
      for (final tone in DievasBadgeTone.values) {
        await tester.pumpWidget(_harness(DievasBadge(label: tone.name, tone: tone)));
        expect(tester.takeException(), isNull);
      }
    });
    ...
    testWidgets('renders all style × tone combinations', (tester) async {
      for (final style in DievasBadgeStyle.values) {
        for (final tone in DievasBadgeTone.values) {
          await tester.pumpWidget(_harness(DievasBadge(label: '$style-$tone', style: style, tone: tone)));
          expect(tester.takeException(), isNull);
        }
      }
    });
    ...
    testWidgets('renders with leading icon', (tester) async {
      await tester.pumpWidget(_harness(const DievasBadge(label: 'Active', leadingIcon: Icon(Icons.circle))));
      expect(tester.takeException(), isNull);
    });
  });
}
